 ...
 * filter_array_values
 * 
 * Get the array and match the ids to get its equivalent names
 * 
 * @param Array $list
 * @param Mixed $value
 * 
 * @return String
 */
function filter_array_values(array $list, $value) {
 ...
 * @return String
 */
function get_equivalent(array $list, $column, $match, $value) {
    foreach($list as $item) {
        if($item[$column] == $match) {
            return $item[$value];
        }
    }
}
...
/**
 * Generate a dynamic breadcrumb to be displayed on each page
 * 
 * @param String        $title
 * 
 * @return String
 */
function breadcrumb($title = null, array $page = [], array $main = [], array $previous = []) {
    $pages_list = "";
    if(!empty($page)) {
        $pages_list .= "<li class='breadcrumb-item'><a href='".base_url($page[0])."'>{$page[1]}</a></li>";
    }
    if(!empty($main)) {
        $pages_list .= "<li class='breadcrumb-item'><a href='".base_url($main[0])."'>{$main[1]}</a></li>";
    }
    if(!empty($previous)) {
        $pages_list .= "<li class='breadcrumb-item'><a href='".base_url($main[0].'/'.$previous[0])."'>{$previous[1]}</a></li>";
    }
    
    return '
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">'.($title ? $title : 'Dashboard').'</h4>
        <div class="page-title-right">
            <ol class="breadcrumb m-0">
                '.$pages_list.'
                <li class="breadcrumb-item active">'.($title ? character_limiter($title, 25) : 'Dashboard').'</li>
            </ol>
        </div>
    </div>';
}
...
/**
 * Determine whether it is morning or afternoon or evening
 * 
 * Will prepare random quotes/messages to share on every visit to the dashboard
 * 
 * @return String
 */
function welcome_msg() {
    ...
    // return the greeting and the message for the day
    return [
        'greeting' => "Good " . $dayTerm,
        'message' => "Here's what's happening with the Church today."
    ];
}
 ...
 * @return String
 */
function countries_list($country_code = null) {
